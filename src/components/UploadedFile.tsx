...
const UploadedFile = ({source,status}:UploadedFileProps) => {
  ...
  useEffect(() => {
    ...
    console.log(status)
  return (
          <div className='flex items-center justify-between w-full py-3 px-4 border border-blue-100 rounded-md bg-slate-50'>
            <div className="flex items-center justify-center ">
                <img className='w-5 mr-5' src={icon} alt={sourceType}/>
                <p className="text-xs font-medium ">{text}</p>
            </div>
            {status === UploadStatus.UPLOADING && <Spinner/> }
            {status === UploadStatus.SUCCESS && <FaCheck className="w-5 text-green-500" />}
            {status === UploadStatus.FAILED && <p className="text-red-500 text-sm ">Failed</p>}
          </div>
  )
}
